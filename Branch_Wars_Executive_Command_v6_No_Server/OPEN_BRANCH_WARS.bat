@echo off
cd /d "%~dp0"
title Branch Wars - Executive Command v7.1
...
for /f "usebackq delims=" %%I in (`powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$ips=@();" ^
  "Get-NetRoute -DestinationPrefix '0.0.0.0/0' ^| Sort-Object RouteMetric ^| ForEach-Object { Get-NetIPAddress -InterfaceIndex $_.ifIndex -AddressFamily IPv4 ^| ForEach-Object { $ips += $_.IPAddress } };" ^
  ...
  "($ips ^| Where-Object { $_ -notlike '127.*' -and $_ -notlike '169.254.*' } ^| Select-Object -First 1)"`) do set "LANIP=%%I"
