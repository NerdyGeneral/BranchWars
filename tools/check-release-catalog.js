...
function zipEntries(bytes){
 ...
 for(let i=0;i<count;i++){
  ...
  const parts=name.split('/');assert.equal(parts.length,2,'Unexpected ZIP nesting');assert(inventory.includes(parts[1]));assert(!entries.has(parts[1]));
  ...
  const payload=bytes.subarray(start,start+compressed);assert([0,8].includes(method));const data=method===0?payload:zlib.inflateRawSync(payload,{maxOutputLength:10*1024*1024});assert.equal(data.length,size);entries.set(parts[1],data);
 }
 assert.deepEqual([...entries.keys()].sort(),inventory);return entries;
}
...
console.log(JSON.stringify({passed:true,preservedMain:catalog.preservedMain.commit,versions:catalog.versions.map(v=>v.id),packageFiles:12,exactZipEntries:12,artifacts:Object.keys(catalog.artifacts).length,localLinks:links,scope:'Integrity and unchanged default game; no new gameplay or physical multiplayer acceptance.'},null,2));
