// Provisional Group4 continuation. Identity, ordinary construction/conversion and
...
const FacilityLifecycle = (() => {
  ...
  function lifecyclePrepare(p,plan,context){
    lifecycleContext(context,'plan');lifecycleValidate(p,context.cycle,true);
    ...
    const q=lifecycleQuote(p,plan,context);if(!q.eligible)throw Error(q.reason);
  ...
  function lifecycleAdvance(p,context){
    ...
    for(const o of lifecycleOffices(next).filter(active)){const r=b.records[o.id].renovation;if(!r||r.readyCycle!==null)continue;
      if(usedCapacity+1>context.freeExecution){events.push({type:'renovation.stalled',officeId:o.id});continue;}
      usedCapacity++;r.work=Math.min(RULES.renovationWork,r.work+context.workRate);if(r.work===RULES.renovationWork)r.readyCycle=context.cycle+1;
      events.push({type:'renovation.progress',officeId:o.id,work:r.work,readyCycle:r.readyCycle});
    }
    b.lastAdvancedCycle=context.cycle;b.lastAdvanceCapacity=usedCapacity;return {owner:next,events,usedCapacity};
  }
