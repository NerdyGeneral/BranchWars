// Experimental owner-only adapter. Loaded inside the engine closure in tests;
// no monkey-patched prices, forecasts, extra saved feature map or implicit upgrade.
...
function facilityLifecyclePlanningContext(v,p,draft,budget=planBudget(p,draft)) {
 ...
 const limit=Math.min(p.stats.cash-reserve,budget.capitalBudget??p.stats.cash);
 const freeCash=Math.max(0,Math.floor(limit-otherSpend));
 ...
 return {owner,commitment,remaining:limit-otherSpend-commitment.total,
...
function lifecycleInstructionQuote(v,p,draft={}) {
 ...
 try{
  ...
  try{
   ...
   if(staged.freeCapacity<0)result.status={eligible:false,reason:'Shared project and facility work exceeds execution capacity.'};
