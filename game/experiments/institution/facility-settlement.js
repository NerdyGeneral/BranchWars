// N-05 integration adapter under review, not in the portable manifest yet.
...
const FacilitySettlement=(()=>{
  ...
  function facilitySettlementInitialize(g){
    if(g.financialGroupVersion!==4)return copy(g);
  ...
  function facilitySettlementAdvance(g,contextFactory){
    ...
    return {game:next,events,usedCapacity};
  }
  ...
  function facilitySettlementValidate(g){
    ...
    if(g.financialGroupVersion!==4){if(world!==undefined||g.players.some(p=>p.facilityLifecycle!==undefined))throw Error('Unversioned facility lifecycle.');return;}
    ...
    const total=world.renovationPaid+world.maintenancePaid;
