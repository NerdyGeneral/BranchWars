// Experiment only: the versioned engine adapter owns prices, shared staff,
// authored proximity and licenses. Not included in the portable manifest yet.
let lifecycleUi={owner:null,campaign:null,signature:null,office:null,open:false,form:null,revision:0,notice:''};
...
function prepareLifecycleStaff(v,signature=JSON.stringify(draft),campaign=game||view){
  ...
  try{const policy=lifecycleReadForm(),candidate=JSON.parse(JSON.stringify(draft));candidate.facilityLifecyclePolicy=policy;
    ...
    const review=lifecycleReview(v,next);if(!review.status.eligible)throw Error(review.status.reason);
    lifecycleUi.form=next;lifecycleUi.notice='Staffing proposal prepared for '+changes.length+' offices; no hiring or plan changes applied. Review the allocations, then Stage. Unused quarter-FTE: '+
...
function renderFacilityLifecycle(v){
  ...
  if(!p.facilityLifecycle){mount.innerHTML='';mount.classList.add('hidden');lifecycleUi={owner:null,campaign:null,signature:null,office:null,open:false,form:null,revision:lifecycleUi.revision+1,notice:''};return;}
  mount.classList.remove('hidden');
  if(lifecycleUi.owner!==p.id||lifecycleUi.campaign!==campaign)lifecycleUi={owner:p.id,campaign,signature:null,office:null,open:false,form:null,revision:lifecycleUi.revision,notice:''};
  if(lifecycleUi.signature!==signature){lifecycleUi.form=JSON.parse(JSON.stringify(draft.facilityLifecyclePolicy||E.defaultFacilityLifecyclePlan(p)));lifecycleUi.signature=signature;lifecycleUi.notice='Form changes remain unsubmitted until Stage.';}
  ...
  mount.innerHTML='<details id="facilityLifecycleDesk"'+(lifecycleUi.open?' open':'')+'><summary>OFFICE CONDITION &amp; STAFFING · '+integer(offices.length)+' operating sites</summary><section class="credit-policy group-credit-policy">'+
    ...
    (office?'<label for="lifecycleOffice">Inspect existing office<select id="lifecycleOffice">'+offices.map(o=>'<option value="'+esc(o.id)+'"'+(o.id===office.id?' selected':'')+'>'+esc((v.territories[o.market]?.name||o.market)+' · '+E.FacilityLifecycle.CATALOG[o.model].name+' · '+o.id)+'</option>').join('')+'</select></label>'+
      '<p class="micro">Age '+integer(record.ageMonths)+' months · deferred wear '+Number(record.deferredWearBp/100).toFixed(2)+' condition points · '+integer(record.renovations)+' completed renovations.</p>'+
