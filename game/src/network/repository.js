...
async function ghJoinRoom(){
 try{setStartMessage('');const invite=unpack($('#ghJoinCode').value,'BW7-ROOM-');const name=validName('#ghGuestName');
  ...
  ghSend({type:'hello',lobbySupported:1,pilotSupported:11,managementSupported:1,relationshipSupported:1,customerDemandSupported:2,regionalGrowthSupported:1,advertisingSupported:1,productProgramsSupported:1,segmentDepositsSupported:1,creditPerformanceSupported:1,customerOwnershipSupported:1,workforceSupported:1,color:p2pConfig.color,name,doctrine:'commercial'});
 }catch(e){gh.active=false;show('#startScreen');setMode('gh');setStartMessage(e.message);setConnection(e.message,'bad')}}
