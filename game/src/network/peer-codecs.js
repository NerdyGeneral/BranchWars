...
function departmentPeerStatus(settings=currentFeatureSource()){
 if(![6,7,8].includes(settings?.financialGroupVersion)&&!(p2pRole==='guest'&&hostStaffingEvidence?.required))return {compatible:true,pending:false,reason:''};
...
function receiveDepartmentPeer(settings){
 ...
 if(p2pRole==='guest'){
  if(!hostStaffingEvidence&&[6,7,8].includes(settings?.financialGroupVersion))hostStaffingEvidence={generation:featureConnectionGeneration};
  if(hostStaffingEvidence)hostStaffingEvidence.required=[6,7,8].includes(settings?.financialGroupVersion);
 }
...
function capturePeerFeatures(message){
 ...
 if(!featureChallenge&&message.featureChallenge===undefined&&
    [3,4,5,6,7,8].includes(currentFeatureSource().financialGroupVersion)&&caps.financialGroupSupported===2)
