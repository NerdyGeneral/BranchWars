...
function lobbyOptions(){
 ...
 return {scope:['town','regional','state','national'].includes(c.scope)?c.scope:'national',
  ...
  managementVersion:c.managementVersion||0,customerDemandVersion:c.customerDemandVersion||0,...(c.relationshipOffersVersion?{relationshipOffersVersion:c.relationshipOffersVersion}:{}),...(c.regionalGrowthVersion?{regionalGrowthVersion:c.regionalGrowthVersion}:{}),...(c.advertisingVersion?{advertisingVersion:c.advertisingVersion}:{}),...(c.productProgramsVersion?{productProgramsVersion:c.productProgramsVersion}:{}),...(c.segmentDepositsVersion?{segmentDepositsVersion:c.segmentDepositsVersion}:{}),...(c.creditPerformanceVersion?{creditPerformanceVersion:c.creditPerformanceVersion}:{}),...(c.customerOwnershipVersion?{customerOwnershipVersion:c.customerOwnershipVersion}:{}),...(c.workforceVersion?{workforceVersion:c.workforceVersion}:{})};
}
...
function renderLobby(){
 ...
 $('#lobbyRules').textContent=(settings.campaignRulesVersion===1?'Regional Rivalry pilot · 2 regions / 6 markets (overrides size).':'Legacy campaign rules · open-ended.')+
  (settings.serviceExpansionVersion?' Expanded services preview.':'')+(settings.managementVersion?' Living institution preview.':'')+(settings.customerDemandVersion?' Customer needs preview.':'')+(settings.workforceVersion?' Specialist workforce preview.':'')+(settings.customerOwnershipVersion?' Household ownership preview.':'')+(settings.advertisingVersion?' Advertising attribution preview.':'')+(settings.regionalGrowthVersion?' Regional growth preview.':'')+(settings.relationshipOffersVersion?' Relationship offers preview.':'')+(settings.productProgramsVersion?' Product programmes preview.':'')+(settings.segmentDepositsVersion?' Segment deposits preview.':'')+(settings.creditPerformanceVersion?' Credit performance preview.':'')+' Preview choices were set by the host when opening the room.';
