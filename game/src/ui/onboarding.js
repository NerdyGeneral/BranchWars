...
function onboardingUiLocked(v){return v.me.submitted||v.gameOver||[6,7,8].includes(v.financialGroupVersion)&&(draftOwner!==v.me.id||lastCycle!==v.cycle||gh.active&&gh.paused);}
...
function onboardingContent(v) {
 ...
 try {
  ...
  if(![6,7,8].includes(v.financialGroupVersion))return '<section class="onboarding-desk"><h3>APPLICATIONS &amp; ONBOARDING</h3>' + caveat + '<p class="notice">Draft quote unavailable: ' + esc(error.message) + '</p>' + actual + '</section>';
...
function stageOnboarding(v, field, value, sourceKey = onboardingSourceKey(v),campaign=game||view) {
 ...
 if([6,7,8].includes(live.financialGroupVersion)&&(campaign!==(game||view)||draftOwner!==live.me.id||lastCycle!==live.cycle||gh.active&&gh.paused))return false;
 try {
  ...
  const pauseRepair=[6,7,8].includes(live.financialGroupVersion)&&field==='share'&&next.onboardingPolicy.share===0&&(draft.onboardingPolicy||live.me.onboarding.policy).share>0;
  ...
  if([6,7,8].includes(live.financialGroupVersion)&&!pauseRepair)E.departmentCustomerPreview(live.me,live,next);
  const fresh = onboardingLive(live);
  if (!fresh || fresh.me.submitted || fresh.gameOver || sourceKey !== onboardingSourceKey(fresh)||[6,7,8].includes(live.financialGroupVersion)&&campaign!==(game||view)) return false;
