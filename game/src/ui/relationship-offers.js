...
function relationshipOfferUiLocked(v){return v.me.submitted||v.gameOver||v.financialGroupVersion===6&&(draftOwner!==v.me.id||lastCycle!==v.cycle||gh.active&&gh.paused);}
...
function relationshipOfferContent(v,productPreview){
 if(v.financialGroupVersion!==6)return relationshipOfferDeskBody(v,productPreview);
...
function relationshipOfferDeskBody(v, productPreview, prepared=null) {
 ...
 return '<section class="relationship-offer-desk"><h3>EXISTING CUSTOMER OFFERS</h3><p class="small muted">Offer an existing customer segment a better-fitting product. This switches eligible balances between products; it does not add multiple-product ownership, households or deposit funds.</p>' +
  ...
  '<p class="micro">'+(quote.staffingVersion===2?'The share reserves physical Retail time after retention. Additional function allocations and purchased work can add capacity. Remaining sales capacity already accounts for onboarding and all other reservations; it is not deducted again. Vendor fees are already included once in the shared plan, separately from conversion expense. Expertise improves output without creating employees. ':'Assigned time comes from Retail sales time after the Customers retention reserve. It leaves less time to acquire outside customers. ')+'Existing staffing can process up to ' + integer(E.RELATIONSHIP_OFFER_CAPACITY) + ' equivalents per effective banker, capped at 10% of eligible equivalents each month. Cash and capital limits can reduce or pause conversion expense.</p>' +
