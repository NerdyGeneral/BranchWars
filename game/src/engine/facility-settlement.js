...
const FacilitySettlement=(()=>{
  ...
  function facilitySettlementInitialize(g){
    if(g.financialGroupVersion!==5)return copy(g);
  ...
  function facilitySettlementValidate(g){
    ...
    if(g.financialGroupVersion!==5){if(world!==undefined||g.players.some(p=>p.facilityLifecycle!==undefined))throw Error('Unversioned facility lifecycle.');return;}
