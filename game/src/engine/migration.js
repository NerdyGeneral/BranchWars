...
function repairSavedRivalry(g){
 ...
 g.version=g.customerOwnershipVersion===1?'8.6':g.workforceVersion===1?'8.5':g.customerDemandVersion===2?'8.4':g.customerDemandVersion===1?'8.3':g.managementVersion?'8.2':'8.1';
 g.maxCycles=null;
...
function migrateCampaign(g){
 ...
 if(g.customerOwnershipVersion!==undefined&&g.version!=='8.6')throw Error('Household ownership requires a v8.6 save.');
 if(g.workforceVersion!==undefined&&g.version!==(g.customerOwnershipVersion===1?'8.6':'8.5'))throw Error('Specialist workforce requires a v8.5 save.');
 if(!['6.0','7.0','7.1','8.0','8.1','8.2','8.3','8.4','8.5','8.6'].includes(g.version))throw Error('Only v6.0 through v8.6 saves are supported.');
