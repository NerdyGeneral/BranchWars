...
function validateLedger(g){
 ...
 if(!uint(pruned))fail();
const sources={...([6,7,8].includes(g.financialGroupVersion)?{'departments.dispatch':'deliverDepartmentFunctions','departments.functions':'prepareDepartmentFunctions','departments.delivery':'finishDepartmentFunctions'}:{}),...([5,6,7,8].includes(g.financialGroupVersion)?{'facilities.lifecycle':'prepareFacilityLifecycle','facilities.renovation':'advanceFacilityLifecycle','facilities.maintenance':'settleFacilityLifecycle','facilities.renewal':'activateFacilityLifecycle'}:{}),...([4,5,6,7,8].includes(g.financialGroupVersion)?{'facilities.instructions':'prepareFacilityInstructions','facilities.progress':'advanceFacilityInstructions','facilities.activation':'activateFacilityInstructions','departments.leadership':'settleDepartmentLeadership','departments.experience':'settleDepartmentExperience'}:{}),...([3,4,5,6,7,8].includes(g.financialGroupVersion)?{'group.agency':'settleAgency'}:{}),...([2,3,4,5,6,7,8].includes(g.financialGroupVersion)?{'companies.settlement':'settleCorporateEconomy','companies.contracts':'finishCorporateEconomy'}:{}),...([1,2,3,4,5,6,7,8].includes(g.financialGroupVersion)?{'group.capital':'settleGroupCapital','group.portfolio':'applyGroupPortfolio'}:{}),'customers.onboarding':'applyOnboardingPolicy','customers.offers':'applyRelationshipOfferPolicy','economy.external':'settleRegionalGrowth','advertising.policy':'applyAdvertisingPolicy','products.policy':'applyProductProgramPolicy','resolution.start':'resolveCycle','operations.result':'operate',decision:'applyDecision','capital.assistance':'applyCapitalRequest','competition.actions':'resolveCompetitiveActions','project.start':'startProject',operations:'operate','funding.deleverage':'deleverage','competition.deposits':'depositContest','funding.settlement':'settleFunding',relationships:'resolveOpportunities','markets.competition':'simulateMarkets','markets.exit':'resolveMarketExits','markets.dividend':'franchiseDividends','project.advance':'advanceProjects','risk.consequences':'consequences','research.investment':'applyInvestments','staff.hiring':'applyHiring','staff.training':'settleWorkforceTraining',milestones:'awardMilestones','campaign.ending':'evaluateStrategicEnd'};
 const fields=new Set([...([6,7,8].includes(g.financialGroupVersion)?['departmentFunctions']:[]),...([5,6,7,8].includes(g.financialGroupVersion)?['facilityLifecycle']:[]),...([4,5,6,7,8].includes(g.financialGroupVersion)?['facilityNetwork','departmentOffice']:[]),...([3,4,5,6,7,8].includes(g.financialGroupVersion)?['groupAgency']:[]),...([2,3,4,5,6,7,8].includes(g.financialGroupVersion)?['corporateBanking']:[]),...([1,2,3,4,5,6,7,8].includes(g.financialGroupVersion)?['groupCapital']:[]),'onboarding','relationshipOffers','regionalFlow','advertising','productPrograms','householdBook','workforce','customerRelationships','depositProducts','fundingCovenant','creditSummary','marketBook','regionalOperations','projects','branches','facilities','facilityMarkets','capability','specializations','allocation','achievements','marketingTurns','fundingGap','distress','capitalRequests','boardConcessions','capitalRestriction','hiresTotal','turnEffects','markets','ending']);
...
function ledgerPosition(g,p,index){return ledgerCopy({...(p.departmentFunctions?{departmentFunctions:{policy:p.departmentFunctions.policy,lastCycle:p.departmentFunctions.lastCycle,paid:p.departmentFunctions.paid,rows:p.departmentFunctions.report?.rows||null,delivery:p.departmentFunctionDelivery?{cycle:p.departmentFunctionDelivery.cycle,rows:p.departmentFunctionDelivery.report.rows,remainingPools:p.departmentFunctionDelivery.report.remainingPools}:null}}:{}),...(p.facilityLifecycle?{facilityLifecycle:p.facilityLifecycle}:{}),...(p.facilityNetwork?{facilityNetwork:p.facilityNetwork}:{}),...(p.departmentOffice?{departmentOffice:p.departmentOffice}:{}),...(p.agency?{groupAgency:{status:p.agency.status,staff:p.agency.staff,policy:p.agency.policy,accounts:p.agency.book.accounts,retainedEarnings:p.agency.book.retainedEarnings,report:p.agency.report,investmentBasis:p.financialGroup.investmentBasis}}:{}),...(p.corporate?{corporateBanking:{receivables:p.accounting.accounts.receivables,report:p.corporate.report,companies:g.companyEconomy.companies.map(c=>({id:c.id,cash:c.book.accounts.cash,debt:c.book.accounts.debt,payables:c.book.accounts.payables,bankClaim:c.bankArrears[index],closed:!!c.resolution}))}}:{}),...(p.financialGroup?{groupCapital:{accounts:p.financialGroup.parent.accounts,retainedEarnings:p.financialGroup.parent.retainedEarnings,allocation:p.creditPortfolio.allocation,report:p.financialGroup.report}}:{}),...(p.onboarding?{onboarding:p.onboarding}:{}),...(p.relationshipOffers?{relationshipOffers:p.relationshipOffers}:{}),...(p.advertising?{advertising:{policy:p.advertising.policy,lastCycle:p.advertising.lastCycle,report:p.advertising.report?{spent:p.advertising.report.spent,depositIntake:p.advertising.report.depositIntake,householdIntake:p.advertising.report.householdIntake,assistedDeposits:p.advertising.report.assistedDeposits,assistedHouseholds:p.advertising.report.assistedHouseholds}:null}}:{}),...(p.productPrograms?{productPrograms:p.productPrograms}:{}),...(p.householdBook?{householdBook:p.householdBook}:{}),...(p.workforce?{workforce:p.workforce}:{}),...(p.customerRelationships?{customerRelationships:p.customerRelationships}:{}),depositProducts:depositLedgerSummary(p,g),fundingCovenant:p.fundingCovenant||null,creditSummary:creditSummary(p),marketBook:p.marketBook||null,regionalOperations:p.regionalOperations||null,stats:p.stats,projects:p.projects,branches:p.branches,facilities:p.facilities,facilityMarkets:p.facilityMarkets,capability:p.capability,specializations:p.specializations,allocation:p.allocation,achievements:p.achievements,marketingTurns:p.marketingTurns,fundingGap:p.fundingGap,distress:p.distress,capitalRequests:p.capitalRequests,boardConcessions:p.boardConcessions,capitalRestriction:p.capitalRestriction,hiresTotal:p.hiresTotal,turnEffects:p.turnEffects,markets:Object.fromEntries(Object.entries(g.territories).map(([key,t])=>[key,{share:t.shares[index],exited:!!(t.exited&&t.exited[index]),exitStreak:t.exitStreak&&t.exitStreak[index]||0}])),ending:{gameOver:g.gameOver,winnerId:g.winnerId,endReason:g.endReason||null}})}
