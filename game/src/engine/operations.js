...
function prepareOperatingForecast(p,plan){
 ...
 if(copy.workforce){applyWorkforcePolicy(copy,plan.workforcePolicy);const q=planBudget(copy,plan);copy._workforceReserved=q.total-(q.training||0)-(q.advertising||0)-(q.relationshipOffers||0)-(q.onboarding||0)}
...
function projectOperatingForecast(copy){
...
function operatingPreview(p,plan,economy){
 if(p.departmentOffice){
  ...
  const normalized=departmentCopy(plan);normalizeDepartmentPlan(p,normalized);
  const prepared=prepareOperatingForecast(p,normalized),facilityCost=facilityDraftSpend(prepared,normalized),
  ...
  return projectOperatingForecast(copy);
 }
 return projectOperatingForecast(finishOperatingForecast(prepareOperatingForecast(p,plan),economy));
}
