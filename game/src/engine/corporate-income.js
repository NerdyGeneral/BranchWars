...
function withCorporateForecast(g,fn){
  if(![2,3,4,5].includes(g.financialGroupVersion))return fn();
...
function initializeCorporateEconomy(g){
  if(![2,3,4,5].includes(g.financialGroupVersion))return;
  g.companyEconomy=CompanyFinance.opening(g.serviceAgreements.map(c=>({market:c.market,baseFee:SERVICE_TYPES[c.kind].fee})));
  if([3,4,5].includes(g.financialGroupVersion))g.companyEconomy=CompanyFinance.withAgency(g.companyEconomy);
...
function validateCorporatePlayer(p,world,month,groupVersion){
  ...
  if(p.accounting.version!==([4,5].includes(groupVersion)?3:2)||p.accounting.accounts.receivables!==world.companies.reduce((n,x)=>n+x.bankArrears[c.index],0))
...
function validateCorporateSave(g){
  if(![2,3,4,5].includes(g.financialGroupVersion)){
  ...
  if(g.companyEconomy.version!==([3,4,5].includes(g.financialGroupVersion)?3:2))throw Error('Company rules do not match the campaign.');
...
function validateCorporateView(view){
  if(![2,3,4,5].includes(view.financialGroupVersion)){
  ...
  if(companyStatement.world.version!==([3,4,5].includes(view.financialGroupVersion)?3:2))throw Error('Company view rules do not match the campaign.');
