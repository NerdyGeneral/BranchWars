...
function creditTerms(p,g){
 ...
 return {product,remaining:CREDIT_TERMS[product],rate:Math.round(.0047*(.8+(g&&g.economy?g.economy.rate:3.75)/12)*option.spread*1000000),risk:Math.round(option.credit*{conservative:.55,balanced:1,growth:1.45}[p.policies.lending]*10000)};
}
function compactCredit(p){
 const grouped=new Map();
 for(const c of p.creditBook.cohorts){if(!c.principal)continue;const key=[c.market,c.product,c.remaining,c.rate,c.risk].join('|');if(grouped.has(key))grouped.get(key).principal+=c.principal;else grouped.set(key,{...c})}
...
function creditSummary(p){
 ...
 return {principal,nextPrincipal:c.reduce((n,x)=>n+Math.ceil(x.principal/x.remaining),0),monthlyInterest:Math.round(c.reduce((n,x)=>n+x.principal*x.rate/1000000,0)),weightedMonths:principal?c.reduce((n,x)=>n+x.principal*x.remaining,0)/principal:0,products:Object.fromEntries(Object.keys(CREDIT_TERMS).map(k=>[k,c.filter(x=>x.product===k).reduce((n,x)=>n+x.principal,0)]))};
}
function repayCredit(p){
 let total=0;
 for(const c of p.creditBook.cohorts){const n=Math.ceil(c.principal/c.remaining);c.principal-=n;c.remaining--;p.marketBook.markets[c.market].loans-=n;total+=n}
 compactCredit(p);
