...
_Generated from `BRANCH_WARS.html` by `tools/build_reference.js`. Do not edit this file by hand — edit `tools/reference-template.md` or the engine, then run `node tools/build_reference.js`._
...
|---|---|---|---|
| town | TOWN CAMPAIGN | 4 | 0 |
| regional | REGIONAL CAMPAIGN | 6 | 1 |
| state | STATEWIDE CAMPAIGN | 8 | 2 |
| national | NATIONAL LEGACY CAMPAIGN | 12 | 3 |
...
| rate | RATE SHOCK |
| regulatory | REGULATORY SIEGE |
| growth | GROWTH FRENZY |
...
|---|---|---|---|---|
| ACT I | LOCAL RIVALRY | 2.5% | 1x | Build the franchise and establish defensible market positions. |
| ACT II | REGIONAL EXPANSION | 4.5% | 1.25x | Strategic platforms intensify competition and weak franchises begin to retreat. |
| ACT III | CONSOLIDATION WAR | 7.5% | 1.55x | Markets can be lost permanently, funding runs accelerate, and vulnerable rivals face a buyout. |
...
|---|---|---|---|
| 0 | WELL CAPITALIZED | >= 8% | No regulatory restrictions. |
| 1 | ENHANCED SUPERVISION | >= 6% | Branch and acquisition projects are suspended. |
| 2 | CONSENT ORDER | >= 4% | Deposit growth is capped and the loan book must shrink each cycle. |
| 3 | UNDERCAPITALIZED | >= 2% | All new projects are barred and the franchise is visibly weakened. |
| 4 | CRITICALLY UNDERCAPITALIZED | below all thresholds | Three consecutive cycles here forces receivership. |
...
|---|---|---|---|---|---|
| BROAD EXPANSION | 4.25% | 1.22x | 0.92x | 0.72x | Confidence and loan demand are strong; credit losses are subdued. |
| STEADY GROWTH | 3.75% | 1x | 1x | 1x | Production and funding markets are broadly balanced. |
| TIGHT MONEY | 6.25% | 0.88x | 1.28x | 1.08x | Funding is expensive and deposit competition is intense. |
| CREDIT DOWNTURN | 3% | 0.72x | 0.9x | 1.65x | Borrowers are cautious and losses are climbing. |
| EARLY RECOVERY | 3.5% | 1.1x | 0.96x | 1.18x | Confidence is returning, but weak credits still require attention. |
...
| Role | Name | What it drives |
|---|---|---|
| service | Retail & Service | Households, deposits, reputation |
| business | Business Banking | Business, treasury, merchant |
| lending | Lending | Loan production and revenue |
| operations | Operations & Risk | Controls and project execution |

**Recruiting** — cost rises with headcount, and hiring several at once dilutes morale.

```js
function hireCost(p,count){let total=0;
const staff=p.stats.staff;
for(let i=0;i<count;i++)total+=Math.round(HIRE_BASE_COST*(1+(staff+i)/45));
return total}
```
...
| Department | Specialist | Recruiting premium | Monthly salary premium | Workload effect |
|---|---|---|---|---|
| service | Retail advisers | $30K | $4,000 | Retail acquisition and service coverage; stronger goodwill when workload is covered. |
| business | Relationship bankers | $45K | $5,000 | Commercial sales or signed-service delivery, divided between the two assignments. |
| lending | Credit analysts | $50K | $6,000 | Loan origination capacity. More production also requires funding and capital. |
| operations | Risk specialists | $60K | $6,500 | Project execution and credit-loss controls; not sales in every department. |
...
```js
function depositPull(p,k,t){const a=p.allocation,competitive=(p.focus===k?(p.turnEffects.depositAttack||0):0)+(p.turnEffects.depositDefense||0),models=marketFacilities(p,k),retail=models.filter(x=>x==='retail').length,digital=models.filter(x=>x==='digital').length;
return (p.branches[k]||0)*(1.5+strategyLevel(p,'network')*.12)+retail*1.5+digital*.55+(p.focus===k?1.8:0)+a.service*.9+p.stats.reputation/22+p.stats.digital/55+strategyLevel(p,'digital')*.1+(p.doctrine==='community'?.375:0)+(p.doctrine==='commercial'?.25:0)+(p.doctrine==='efficiency'?.775:0)+(p.doctrine==='people'?.6:0)+(p.policies.deposit==='aggressive'?.75:p.policies.deposit==='margin'?-.4:0)+(p.marketingTurns>0?1.2:0)+(t.specialties.includes('service')?1:0)+competitive-(tierRank(p)>=3?2.5:0)}
```
...
|---|---|---|---|---|---|
| Downtown | 0 | 1 | 15 | service, corporate | Deposits + executive visibility |
| Northside | 0 | 1 | 15 | service, digital | Households + digital adoption |
| Industrial Corridor | 0 | 1 | 18 | business | Treasury + merchant services |
| Suburban Growth Belt | 0 | 1 | 15 | lending, people | Consumer lending + staffing |
| County Seat | 1 | 5 | 20 | operations, corporate | Controls + public visibility |
| University District | 1 | 5 | 18 | digital, people | Talent + technology |
| Metro Core | 2 | 10 | 24 | service, business | Commercial + wealth scale |
| State Capital | 2 | 14 | 28 | corporate, operations | Influence + statewide prestige |
| Agricultural Heartland | 3 | 18 | 22 | lending, people | Seasonal credit + relationship loyalty |
| Innovation Hub | 3 | 22 | 30 | digital, business | Technology firms + high-growth treasury |
| Logistics Gateway | 3 | 26 | 32 | business, operations | Trade finance + operating deposits |
| Coastal Resort District | 3 | 30 | 26 | service, corporate | Wealth + seasonal deposits |
...
|---|---|---|---|---|
| deposit | Large Deposit Household | service | $900K | Rate-sensitive deposits and household relationships. |
| business | Operating Company Relationship | business | $750K | Operating deposits, treasury, and merchant services. |
| loan | Commercial Loan Request | lending | $1.10M | Loan production with meaningful revenue and risk. |
| wealth | Private Banking Household | service | $1.25M | Deposits, wealth referrals, and executive visibility. |
| payroll | Employer Payroll Conversion | business | $1.05M | Payroll deposits, employees, and merchant activity. |
| public | Municipal Banking Bid | operations | $1.60M | Public deposits requiring strong controls and influence. |
...
| Key | Name | Cost | Cycles | Capacity | Kind | Facility | Needs a market | Effect |
|---|---|---|---|---|---|---|---|---|
| branch | Full-Service Financial Center | $560K | 3 | 1.5 | branch | retail | yes | Broad household service, local reputation, and balanced deposit capacity. |
| branchCommercial | Commercial Banking Office | $560K | 3 | 1.5 | branch | commercial | yes | Business development, treasury relationships, and merchant capacity. |
| branchDigital | Digital Advisory Studio | $470K | 2 | 1.25 | branch | digital | yes | Low-cost digital acquisition, advisory service, and household onboarding. |
...
| Key | Name | Status |
|---|---|---|
| hire | Legacy Recruit a Banker | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| technology | Legacy Core Technology Upgrade | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| training | Legacy Staff Development Academy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| capital | Legacy Corporate Capital Request | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| analytics | Legacy Market Analytics Platform | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| wealthDesk | Legacy Private Wealth Desk | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| operationsCenter | Legacy Regional Operations Center | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| roadmapNetwork | Branch Network Strategy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| roadmapDigital | Digital Platform Strategy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| roadmapCommercial | Commercial Bank Strategy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| roadmapOperations | Operational Excellence Strategy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
| roadmapAcquisition | Acquisition Strategy | Refused by plan validation. Kept only so saves with this work in flight still resolve it. |
...
| Key | Name | Cost | Cycles | Capacity | Kind | Facility | Needs a market | Effect |
|---|---|---|---|---|---|---|---|---|
| branchService | Relationship Service Upgrade | $140K | 2 | 1 | branchUpgrade | -- | yes | Upgrade this market (max 2): +15% local deposit onboarding capacity and +0.8 competitive deposit pull per level; +$2K monthly per office. |
| branchAutomation | Branch Workflow Automation | $180K | 3 | 1.5 | branchUpgrade | -- | yes | Upgrade this market (max 2): cut local base facility expense by 12% per level. No free customer growth. |
| branchClose | Consolidate One Office | $40K | 1 | 0.5 | branchClosure | -- | yes | Close the last-opened office in this market. Save its recurring costs, but lose its onboarding capacity and competitive pull. No asset-sale proceeds. |
| deployRewards | Deploy Rewards Checking | $180K | 3 | 2 | productDeployment | -- | no | Requires completed BRANCH NETWORK tier 1. Uses 2 execution capacity; staff shortages stall delivery. Unlocks sales next planning cycle without creating balances or opening the offer automatically. |
| deployHighYield | Deploy High-Yield Savings | $220K | 3 | 2 | productDeployment | -- | no | Requires completed DIGITAL PLATFORM tier 1. Uses 2 execution capacity; staff shortages stall delivery. Unlocks sales next planning cycle without creating balances or opening the offer automatically. |
| contractAdvertising | Targeted Relationship Advertising | $24K | 1 | 1 | contractAdvertising | -- | yes | Four future turns of +3 bid strength for service agreements in the selected market. No guaranteed win, deposits, or broad market bonus. |
| deployPayrollDesk | Deploy Payroll Automation | $160K | 3 | 2 | serviceApplication | -- | no | Network + Operations tier 1. Activate for $4K/turn: payroll bids +2 and payroll servicing costs $1.5K less per contract. No free customers. |
| buildTreasuryDesk | Build Corporate Treasury | $220K | 3 | 2 | serviceApplication | -- | no | Commercial + Digital tier 1. Activate for $6K/turn to compete for treasury mandates. Requires dedicated service capacity; replaces a partner platform if present. |
| partnerTreasuryDesk | Partner for Corporate Treasury | $60K | 1 | 1 | serviceApplication | -- | no | Commercial tier 1. Faster $60K setup, but $18K/turn while active. Treasury eligibility without a Digital build; a later internal build replaces the partner. |
...
**BRANCH NETWORK** (`network`) — _Physical reach, local loyalty, and lower expansion cost._

| Tier | Node | Step cost | Cumulative | Effect |
|---|---|---|---|---|
| Tier 1 | Local Market Playbook | $280K | $280K | Branches cost 8% less and generate stronger local pull. |
| Tier 2 | Hub-and-Spoke Network | $480K | $760K | Branch initiatives finish faster and reinforce service capacity. |
| Tier 3 | Deposit Franchise | $760K | $1.52M | Every branch compounds deposit growth and reputation. |
| Tier 4 | National Charter | $1.18M | $2.70M | Capstone: new branches open with immediate customers and market share. |

Operating models (choose one on reaching tier 1, permanent):

| Key | Model | Effect |
|---|---|---|
| retailDensity | Retail Density | Full-service branches produce more households, deposits, and reputation. |
| regionalHub | Regional Hubs | All branch models build faster and reinforce surrounding market capacity. |

**DIGITAL PLATFORM** (`digital`) — _Scalable acquisition, automation, and channel resilience._

| Tier | Node | Step cost | Cumulative | Effect |
|---|---|---|---|---|
| Tier 1 | Mobile Onboarding | $320K | $320K | Accelerate digital adoption and household acquisition. |
| Tier 2 | Automation Engine | $540K | $860K | Lower operating expense and improve service capacity. |
| Tier 3 | Data-Led Growth | $820K | $1.68M | Improve opportunity conversion and digital deposit growth. |
| Tier 4 | Digital Challenger | $1.26M | $2.94M | Capstone: a high-scale national digital deposit franchise. |

Operating models (choose one on reaching tier 1, permanent):

| Key | Model | Effect |
|---|---|---|
| customerExperience | Customer Experience | Digital adoption converts directly into households and deposits. |
| automation | Back-Office Automation | Technology lowers recurring operating expense and project friction. |

**COMMERCIAL BANK** (`commercial`) — _Business relationships, treasury revenue, and disciplined credit scale._

| Tier | Node | Step cost | Cumulative | Effect |
|---|---|---|---|---|
| Tier 1 | Relationship Teams | $360K | $360K | Grow business and merchant relationships faster. |
| Tier 2 | Treasury Platform | $620K | $980K | Increase fee income from operating companies. |
| Tier 3 | Specialized Credit | $1.01M | $1.99M | Expand loan production with better underwriting support. |
| Tier 4 | Corporate Bank | $1.56M | $3.55M | Capstone: convert commercial scale into major earnings power. |

Operating models (choose one on reaching tier 1, permanent):

| Key | Model | Effect |
|---|---|---|
| treasury | Treasury & Payments | Merchant services and operating-account fee income compound faster. |
| specializedCredit | Specialized Credit | Loan production accelerates with a measured increase in underwriting risk. |

**OPERATIONAL EXCELLENCE** (`operations`) — _Delivery capacity, efficiency, controls, and resilience._

| Tier | Node | Step cost | Cumulative | Effect |
|---|---|---|---|---|
| Tier 1 | Process Redesign | $290K | $290K | Long operating initiatives finish one cycle sooner. |
| Tier 2 | Dual Delivery | $560K | $850K | Adds 1.5 execution capacity per Operations tier; staffing supports more concurrent work. |
| Tier 3 | Lean Operations | $860K | $1.71M | Reduce project costs and recurring operating expense. |
| Tier 4 | Resilient Enterprise | $1.28M | $2.99M | Capstone: exceptional risk control and disruption resilience. |

Operating models (choose one on reaching tier 1, permanent):

| Key | Model | Effect |
|---|---|---|
| lean | Lean Delivery | Staff and facility expense fall while project teams execute more efficiently. |
| resilience | Risk & Resilience | Controls, defensive actions, and disruption recovery become materially stronger. |

**ACQUISITION STRATEGY** (`acquisition`) — _Deal sourcing, integration skill, and rapid market consolidation._

| Tier | Node | Step cost | Cumulative | Effect |
|---|---|---|---|---|
| Tier 1 | Integration Office | $240K | $240K | Lower acquisition cost and integration risk. |
| Tier 2 | Deal Pipeline | $410K | $650K | Competitor book acquisitions close one cycle faster. |
| Tier 3 | Consolidation Playbook | $630K | $1.28M | Acquisitions deliver more deposits, loans, and customers. |
| Tier 4 | Serial Acquirer | $950K | $2.23M | Capstone: acquisitions seize substantially more market share. |

Operating models (choose one on reaching tier 1, permanent):

| Key | Model | Effect |
|---|---|---|
| dealmaker | Dealmaker | Acquisitions cost less and close with stronger market-share impact. |
| integrator | Integration Discipline | Acquired customers and assets transfer more cleanly with less attention. |
...
|---|---|---|---|---|---|
| none | Hold Position | $0 | neutral | None | Take no special competitive action this cycle. |
| depositRaid | Targeted Deposit Campaign | $180K | attack | Liquidity Defense | Concentrate pricing and outreach in the focus market. Digital and Acquisition strategy increase pressure. |
| commercialRaid | Commercial Client Raid | $220K | attack | Relationship Defense | Poach business, merchant, and household relationships. Commercial strategy increases the transfer. |
| talentRaid | Talent Poach | $250K | attack | Retention Package | Attempt to recruit a rival banker. Acquisition strategy and influence improve the attempt. |
| liquidityDefense | Liquidity Defense | $110K | defense | Counters Targeted Deposit Campaign | Protect vulnerable funding and blunt a targeted deposit campaign. Operations strategy strengthens it. |
| relationshipDefense | Relationship Defense | $120K | defense | Counters Commercial Client Raid | Deploy senior bankers to protect commercial and household relationships. |
| retentionDefense | Retention Package | $100K | defense | Counters Talent Poach | Protect staff and improve morale against a talent raid. People-first institutions gain extra resilience. |
| takeoverDefense | Shareholder Defense | $260K | defense | Counters Hostile Buyout | Use influence and counsel to break a hostile-buyout position for this cycle. |
...
const payroll=Math.max(0,Number(p.payrollSpend)||0),built=Math.max(0,Number(p.buildSpend)||0),spendTotal=payroll+built+capTotal,payrollShare=spendTotal>0?payroll/spendTotal:0;
 ...
 let total=0;
for(const k of Object.keys(raw)){raw[k]=Math.max(0,raw[k])+.05;
total+=raw[k]}
 const out={};
...
|---|---|---|
| community | Community Franchise | Reputation and local market influence compound faster. |
| commercial | Commercial Powerhouse | Business, merchant, and loan production receive a permanent edge. |
| digital | Digital Challenger | Technology projects and digital adoption scale faster. |
| efficiency | Efficiency Operator | Operating expenses fall and project execution improves. |
| people | People-First Institution | Morale, training, and staff productivity are more resilient. |
...
|---|---|---|---|
| map | MARKET DOMINANCE | +120 | Control a majority of campaign markets. |
| deposits | DEPOSIT TITAN | +100 | Finish with at least $40M in deposits. |
| commercial | COMMERCIAL POWERHOUSE | +100 | Finish with 105 business relationships and 80 merchant clients. |
| earnings | EARNINGS MACHINE | +100 | Generate at least $2.2M in cumulative earnings. |
| clean | CLEAN BOOKS | +100 | Finish with risk at 18 or less and attention below 30. |
| people | EMPLOYER OF CHOICE | +100 | Finish with morale 82+, staff 10+, and training level 2+. |
| digital | DIGITAL FIRST | +100 | Finish with digital adoption 85+ and technology level 2+. |
| network | PHYSICAL EMPIRE | +100 | Operate a full branch network across the map. |

**Milestones** — one-off awards during play:

| Key | Milestone | Requirement |
|---|---|---|
| firstMarket | Market Leader | Control your first market. |
| scale | Balance-Sheet Scale | Reach $50M in combined deposits and loans. |
| rainmaker | Rainmaker | Win 8 live opportunities. |
| builder | Franchise Builder | Operate 8 branch levels. |
| trusted | Trusted Institution | Reach 82 reputation with risk below 30. |
| digital | Digital Transformation | Reach 80 digital adoption and technology level 2. |
...
|---|---|---|
| CAPABILITY_CAP_PER_CYCLE | $250K | Most that may be put into any one capability lane in a single cycle |
| RECEIVERSHIP_CYCLES | 3 | Consecutive cycles of critical distress before the regulator seizes the bank |
| Base execution capacity | 1.5 | Capacity the executive team carries with no Operations bankers |
...
| Max hires per cycle | 6 | Ceiling on recruiting in one cycle |
...
| Key | Name | Effect |
|---|---|---|
| conservative | Conservative | Lower volume and risk |
| balanced | Balanced | Measured loan growth |
| growth | Growth | High volume and higher risk |

**Capital strategy**

| Key | Name | Effect |
|---|---|---|
| liquid | Build Liquidity | Hold cash, slow expansion, absorb shocks |
| balanced | Balanced | Fund steady growth with normal buffers |
| reinvest | Reinvest | Accelerate growth at higher liquidity risk |

### Product models

**RETAIL FRANCHISE** (`retail`, persistent until changed)

| Key | Option | Modifiers | Description |
|---|---|---|---|
| essential | Essential Banking | customers +20%, deposits -2%, funding -10%, reputation +0% | Low-fee accounts build loyalty and households; slower balance growth. |
| rewards | Rewards Checking | customers +8%, deposits +5%, funding +5%, digital +15% | Card rewards improve acquisition and digital engagement at a moderate cost. |
| highYield | High-Yield Savings | customers -8%, deposits +18%, funding +20%, sensitive -84% | Rapid deposit growth, but expensive and highly rate-sensitive funding. |

**BUSINESS BANKING** (`business`, persistent until changed)

| Key | Option | Modifiers | Description |
|---|---|---|---|
| relationship | Relationship Banking | business +16%, merchant +0%, deposits +4%, fees -4% | More operating-company relationships and durable deposits. |
| treasury | Treasury Management | business +0%, merchant +16%, fees +12% | Merchant services and fee income grow faster; needs Business staffing. |
| entrepreneur | Entrepreneur Platform | business +10%, merchant +8%, digital +12%, risk +6%, fees +0% | Faster relationship acquisition and digital adoption with more volatility. |

**CREDIT PORTFOLIO** (`credit`, persistent until changed)

| Key | Option | Modifiers | Description |
|---|---|---|---|
| mortgage | Mortgage Franchise | loans -5%, credit -26%, spread -2% | Stable, lower-yield loan growth with reduced credit losses. |
| middleMarket | Middle-Market Lending | loans +4%, credit +3%, spread +7% | Balanced commercial production, yield, and underwriting risk. |
| consumer | Consumer Finance | loans +20%, credit +27%, spread +16%, attention +0% | High-volume, high-yield production with greater losses and attention. |

### Executive events

| Key | Event | Situation | Option A | Option B |
|---|---|---|---|---|
| quiet | NORMAL OPERATING MONTH | Corporate has not created an emergency. Yet. | Push production — More momentum, slightly more risk. | Documentation day — Reduce risk and improve controls. |
| ratewar | MARKET-WIDE RATE WAR | Competitors are repricing deposits aggressively. | Match the market — Spend cash for faster deposit growth. | Protect margin — Accept slower growth for stronger profit. |
| staffing | UNEXPECTED STAFFING SHORTAGE | Coverage is thin and lobby traffic is climbing. | Authorize overtime — Protect service at a cash and morale cost. | Operate lean — Save cash but risk reputation. |
| audit | REGULATORY EXAM NOTICE | Controls and documentation are under review. | Full remediation — Spend cash to reduce risk and heat. | Defend current practices — Save cash; results depend on Operations staffing. |
| community | STATEWIDE COMMUNITY WEEK | Every institution is expected to demonstrate local commitment. | Sponsor events — Buy reputation and local influence. | Decline politely — Preserve cash; lose a little visibility. |
| outage | CORE SYSTEM OUTAGE | Service capacity depends on technology and preparation. | Emergency vendor support — Spend cash to contain disruption. | Recover internally — Technology and Operations determine the damage. |
| stretch | NEW CORPORATE STRETCH GOALS | Leadership has discovered a larger number in Excel. | Accept the targets — Gain influence but pressure morale. | Negotiate — Protect morale but risk executive attention. |
| fraud | SUSPICIOUS TRANSACTION PANIC | A complicated relationship has triggered review. | Freeze and investigate — Reduce risk but sacrifice production. | Keep processing — Preserve revenue with compliance exposure. |
| downturn | LOCAL RECESSION WARNING | Credit losses may rise across the market. | Tighten credit — Reduce lending and protect the balance sheet. | Keep lending — Defend growth with higher risk. |
| closure | COMPETITOR BRANCH CLOSURE | Displaced customers are searching for a new institution. | Rapid acquisition campaign — Spend cash to capture relationships. | Selective outreach — Smaller but safer opportunity. |
| talent | REGIONAL TALENT SCRAMBLE | A competitor is recruiting experienced bankers. | Retention bonuses — Spend cash and protect morale. | Hold compensation — Save cash and risk turnover pressure. |
| viral | VIRAL CUSTOMER SUCCESS STORY | The institution can amplify a positive local story. | Amplify it — Spend marketing cash for reputation. | Stay understated — Gain a smaller, free benefit. |
| board | BOARD STRATEGY SESSION | Senior leadership wants a convincing growth narrative. | Present aggressively — Influence and attention both rise. | Present conservatively — Smaller influence gain with less risk. |
| cyber | CYBERSECURITY DRILL | Technology and controls will determine whether this becomes embarrassing. | Bring in specialists — Spend cash for protection. | Use internal teams — Operations and technology carry the burden. |
| coffee | THE COFFEE MACHINE HAS FAILED | Employee engagement has entered a critical phase. | Cater emergency coffee — Small cost, meaningful morale. | Issue a resilience memo — Free, but almost certainly ineffective. |
| manager | SURPRISE REGIONAL MANAGER VISIT | The branch network has one afternoon to look functional. | Polish the presentation — Spend cash for reputation and influence. | Show authentic operations — Results depend on morale and controls. |
| fintech | FINTECH ENTERS THE MARKET | A well-funded app is targeting younger depositors. | Launch a digital sprint — Spend cash; technology magnifies the response. | Double down on relationships — Service and reputation determine retention. |
| merger | RIVAL MERGER RUMORS | Customers and employees are anxious about a competitor combination. | Recruit aggressively — Pursue staff and relationships at a cost. | Stay disciplined — Protect morale and wait for dislocation. |
| losses | CREDIT WATCHLIST EXPANDS | Several borrowers are showing early signs of stress. | Build reserves — Pay now to contain future credit damage. | Work through cases — Lending and Operations staffing determine losses. |
| housing | HOUSING BOOM | New households and developers are racing into growth markets. | Finance the expansion — Strong growth with credit exposure. | Prioritize deposits — Safer household growth and liquidity. |
| publicity | EXECUTIVE INTERVIEW REQUEST | A business journal wants your view of the market. | Take the interview — Influence rises; mistakes attract attention. | Nominate a banker — Develop staff and protect the executive team. |
| storm | REGIONAL WEATHER EMERGENCY | Customers need access, flexibility, and visible leadership. | Open relief centers — Spend cash for loyalty and reputation. | Go digital-first — Technology determines service continuity. |
| activist | ACTIVIST INVESTOR LETTER | Corporate wants a sharper return on capital plan. | Promise efficiency — Profit rises temporarily; morale takes pressure. | Defend the franchise — Influence determines whether patience holds. |
| succession | KEY LEADER SUCCESSION | A respected market president announces retirement. | Promote internally — Training and morale make the transition work. | Hire an outsider — Spend cash for immediate production capacity. |
| vendor | VENDOR CONTRACT RENEWAL | Core operating costs are under negotiation. | Consolidate vendors — Lower expense with execution risk. | Preserve resilience — Pay more to protect operations. |
| expansion | CORPORATE EXPANSION WINDOW | Capital is temporarily available for a visible growth move. | Request the funds — Influence converts into cash and attention. | Protect independence — Build morale and keep heat low. |
...
|---|---|---|---|---|---|
| Deploy Payroll Automation | BRANCH NETWORK + OPERATIONAL EXCELLENCE | $160K | 3 | 2 | $4K |
| Build Corporate Treasury | COMMERCIAL BANK + DIGITAL PLATFORM | $220K | 3 | 2 | $6K |
| Partner for Corporate Treasury | COMMERCIAL BANK | $60K | 1 | 1 | $18K |
...
|---|---|
| Everyday households | 1.1 |
| Digitally active customers | 0.65 |
| Reserve savers | 1.3 |
...
|---|---|---|---|---|---|
| Relationship workout | 55.00000000000001% | 30% | 12% | 30% | $2,500 |
| Balanced collections | 35% | 18% | 24% | 50% | $1,800 |
| Accelerated recovery | 15% | 5% | 50% | 70% | $1,200 |
