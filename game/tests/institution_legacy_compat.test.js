...
const baselinePath=path.join(root,'reports/reference-builds/BRANCH_WARS_agency_group3_c3af45b3.html');
...
for(const rules of [1,2,3])for(const scenario of ['balanced','rate','regulatory','growth'])for(const seed of [0,20]){
 const options=old.previewFeatureSelection({}, {field:'financialGroupVersion',value:rules}).options;
 const config={...options,scenario,seed:'institution-legacy:'+seed,created:1,mode:'hotseat'};
 ...
 assert.equal(games[1].version,({1:'9.0',2:'9.1',3:'9.2'})[rules]);
 assert.equal(games[1].departmentEconomy,undefined);
 for(let month=0;month<3;month++){
  ...
  for(const seat of [0,1]){
   ...
   assert.equal(games[1].players[seat].facilityNetwork,undefined);
   assert.equal(games[1].players[seat].departmentOffice,undefined);
   assert.equal(games[1].lastPlans?.[games[1].players[seat].id]?.departmentPolicy,undefined);
  }
  months++;
 }
 for(const [i,E]of engines.entries()){
 ...
 assert.equal(games[1].financialGroupVersion,rules);assert.equal(games[1].departmentEconomy,undefined);profiles++;
}
console.log(JSON.stringify({passed:true,profiles,months,humanMonths,
 baseline:'reports/reference-builds/BRANCH_WARS_agency_group3_c3af45b3.html',scope:'Exact retained Group 1/2/3 creation, AI/human resolution, RNG, owner views, half-ready resume and rematch; fixtures unchanged.'}));
