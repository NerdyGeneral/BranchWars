...
const peerCaps=E.campaignCapabilities(),peerRules=E.validateCampaignRules(g,'game');
assert.equal(peerCaps.pilotSupported,11);assert.equal(E.peerRulesIssue(peerRules,peerCaps),null);
for(const pilotSupported of [undefined,0,10,12])
 assert.equal(E.peerRulesIssue(peerRules,{...peerCaps,pilotSupported}).field,'campaignRulesVersion','Unsupported Regional Rivalry peers must be refused');
